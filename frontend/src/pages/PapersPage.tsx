...
export default class PapersPage extends Component<IProps, IState> {
  ...
  private fetchNewPaper = () => {
    fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000'}/family/?paper=${encodeURIComponent(this.state.selectedPaper?.title || '')}&distance=${this.state.currentFilter?.maxDistance || 3}&year=${this.state.currentFilter?.minYear || 0}&citations=${this.state.currentFilter?.minCitations || 1}`)
